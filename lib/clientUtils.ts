...
export function _sliceArray<T = any>(
	inputArray: T[],
	arrayCount: number
): T[][] {
	let arrays = [];
	...
	for (let i = 0; i < divideFactor; i++) {
		let tempArray = copyArray.splice(
			0,
			arrayCount >= copyArray.length ? copyArray.length : arrayCount
		);
...
export function _generatePost(): Post {
	...
	const title = `${titleWords[Math.floor(Math.random() * titleWords.length)]} ${
		titleNouns[Math.floor(Math.random() * titleNouns.length)]
	...
	const summary = `${
		summaryWords[Math.floor(Math.random() * summaryWords.length)]
	} ${
		summaryNouns[Math.floor(Math.random() * summaryNouns.length)]
...
export function _deserializePosts(json: string): Post[] {
	let posts: Post[] = JSON.parse(json).map(
		(jsonPost: Post & { date: string }) => ({
			...jsonPost,
...
export async function _getSearches(): Promise<Search[]> {
	const jsonSearches = await (
		await fetch(process.env.NEXT_PUBLIC_SEARCHES_FILE || "")
	).json();
	let searches: Search[] = jsonSearches.map(
		({ slug, title, date }: { slug: string; title: string; date: string }) => {
			return {
				slug,
				title,
				date: new Date(date),
